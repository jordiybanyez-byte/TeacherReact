...
export default function InvitarPage() {
  ...
  const [showCourseSelect, setShowCourseSelect] = useState<string | null>(null);
  ...
  const handleAddToCourse = (studentId: string, courseId: string | null = null) => {
    if (!courseId) {
      setShowCourseSelect(studentId);
      return;
    }
    
    setStudents(students.map(s => 
      s.id === studentId ? { ...s, status: 'pending', selectedCourse: courseId } : s
    ));
    setShowCourseSelect(null);
  };

  const getCourseName = (courseId: string) => {
    const curso = cursosDisponibles.find(c => c.id === courseId);
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className={`text-xs ${
                          ...
                          {student.status === 'invited' ? 'Invitado' : 'Agregado a: ' + getCourseName(student.selectedCourse || '')}
                        </span>
                      </div>
                    </div>
                    ...
                    {student.status === 'pending' ? (
                      <span className={`px-3 py-2 rounded-lg text-sm font-medium ${
                        ...
                        {getCourseName(student.selectedCourse || '')}
                      </span>
                    ) : showCourseSelect === student.id ? (
                      <div className="flex gap-2">
                        <select
                          className={`px-3 py-2 rounded-lg border text-sm ${
                            isDark ? 'bg-gray-600 border-gray-500 text-white' : 'bg-white border-gray-300'
                          }`}
                          onChange={(e) => handleAddToCourse(student.id, e.target.value)}
                          autoFocus
                        >
                          <option value="">Seleccionar curso...</option>
                          {cursosDisponibles.filter(c => c.estado === 'activo').map(curso => (
                        ...
                        <button
                          onClick={() => setShowCourseSelect(null)}
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-lg text-sm font-medium transition bg-green-600 hover:bg-green-700 text-white"
                      >
                        {t.agregarCurso}
                      </button>
                    )}
